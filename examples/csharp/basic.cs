...
{
    class UnicodeEscape/UnescapeExample
    {
        private static readonly string API_KEY = Environment.GetEnvironmentVariable("APIVERVE_API_KEY") ?? "YOUR_API_KEY_HERE";
        private static readonly string API_URL = "https://api.apiverve.com/v1/unicodeescape";

        /// <summary>
        /// Make a GET request to the Unicode Escape/Unescape API
        /// </summary>
        static async Task<JsonDocument> CallUnicodeEscape/UnescapeAPI()
        {
            try
            {
        ...
        {
            Console.WriteLine("📤 Calling Unicode Escape/Unescape API...\n");

            var result = await CallUnicodeEscape/UnescapeAPI();
